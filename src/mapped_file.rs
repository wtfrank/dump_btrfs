...
impl MappedFile {
  pub fn open(file: &Path) -> Result<MappedFile> {
    ...
    let p = unsafe {libc::mmap( 0 as *mut c_void, len, libc::PROT_READ, libc::MAP_PRIVATE, f.as_raw_fd(), 0 )};
    if libc::MAP_FAILED == p { return Err(anyhow!("Failed to map file: {}", std::io::Error::last_os_error())) }
    Ok(MappedFile{pointer: p, len, mapping_size})
  }
  ...
  pub fn at<T>(&self, offset: usize) -> &T {
    if self.len - std::mem::size_of::<T>() <= offset { panic!("access beyond end of file"); }
...
impl Index<usize> for MappedFile {
  ...
  fn index(&self, idx: usize) -> &Self::Output {
    if self.len - std::mem::size_of::<usize>() <= idx{ panic!("access beyond end of file"); }
...
mod tests {
    ...
    fn file_at() -> Result<()>{
      ...
      assert_eq!(*mf.at::<u16>(0), unsafe {std::mem::transmute::<[u8;2], u16>( ['[' as u8, 'p' as u8]  )} );
      assert_eq!(*mf.at::<u16>(1), unsafe {std::mem::transmute::<[u8;2], u16>( ['p' as u8, 'a' as u8]  )} );
